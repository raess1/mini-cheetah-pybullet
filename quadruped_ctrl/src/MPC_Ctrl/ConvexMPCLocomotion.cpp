...
ConvexMPCLocomotion::ConvexMPCLocomotion(float _dt, int _iterations_between_mpc) :
  iterationsBetweenMPC(_iterations_between_mpc),  //控制用  15
  horizonLength(10),
  dt(_dt),  //0.002
...
//设置期望值
void ConvexMPCLocomotion::_SetupCommand(StateEstimatorContainer<float> &_stateEstimator, std::vector<double> gamepadCommand){
  ...
  _x_vel_des = _x_vel_des*(1-filter) + x_vel_cmd*filter;  //一阶低通数字滤波
...
void ConvexMPCLocomotion::run(Quadruped<float> &_quadruped, LegController<float> &_legController, StateEstimatorContainer<float> &_stateEstimator,
  ...
  if(gaitNumber == 1)
    ...
    gait = &walking2;
  else if(gaitNumber == 13)
      gait = &trotting; //
  else if(gaitNumber == 14) 
      gait = &trotting; //
  ...
  for(int i = 0; i < 4; i++) {
    pFoot[i] = seResult.position +
      seResult.rBody.transpose() * (_quadruped.getHipLocation(i) +
          _legController.datas[i].p);
  ...
  if(firstRun)
  {
    ...
    for(int i = 0; i < 4; i++)   //足底摆动轨迹
    {
      ...
      footSwingTrajectories[i].setHeight(0.2);
...
//              offset(1)=0.05;
    ...
    Vec3<float> pRobotFrame = (_quadruped.getHipLocation(i) + offset);   //得到身体坐标系下的hip关节坐标
    ...
    float pfx_rel = seResult.vWorld[0] * (.5 + 0.0) * stance_time +     //_parameters->cmpc_bonus_swing = 0.0
...
/*
  ...
  contact_state = gait->getContactState();
  //_wbc_ctrl = new LocomotionCtrl<float>(_quadruped.buildModel());
  //_wbc_data = new LocomotionCtrlData<float>();

  for(int leg(0); leg<4; ++leg){
    pDes_backup[leg] = _legController.commands[leg].pDes;
    vDes_backup[leg] = _legController.commands[leg].vDes;
    Kp_backup[leg] = _legController.commands[leg].kpCartesian;
    Kd_backup[leg] = _legController.commands[leg].kdCartesian;
    }
...
void ConvexMPCLocomotion::updateMPCIfNeeded(int *mpcTable, StateEstimatorContainer<float> &_stateEstimator, bool omniMode) {
